import type { IAppServerOrchestrator, IAppsRoom } from '@rocket.chat/apps';
...
export class AppRoomsConverter {
	...
	async convertById(roomId: IRoom['_id']) {
		const room = await Rooms.findOneById(roomId);

		return this.convertRoom(room);
	}

	async convertByName(roomName: NonNullable<IRoom['name']>) {
		const room = await Rooms.findOneByName(roomName);

		return this.convertRoom(room);
	}

	convertRoomRaw(room: IRoom | null) {
		if (!room) {
			return undefined;
		}
	...
	async convertAppRoom(room: IAppsRoom, isPartial = false) {
		if (!room) {
			return undefined;
		}
	...
	async convertRoom(originalRoom: IRoom | null) {
		if (!originalRoom) {
			return undefined;
		}
